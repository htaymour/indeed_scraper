...
def get_job_data(URL):
    try:
        ...
        j_id = URL.replace('https://au.indeed.com/viewjob?jk=','')
        ...
        if desc.upper().find(' AU . ') >= 0 : desc = desc.replace(' AU .','AU.').replace(" au .","au.")
...
def get_company_data(company):
    try:
    # global d,soup
        URL = "https://au.indeed.com/cmp/"+ company.replace(' ','-')  + "/reviews"
        logoUrl = ""
        jobsBoardUrl = "https://au.indeed.com/cmp/"+ company.replace(' ','-')
        website = ""
        address = ""
        leader_name = ""
        leader_photo = ""
    ...
    try :
        leadership= ""
        URL = "https://au.indeed.com/cmp/"+ company.replace(' ','-')
        page = requests.get(URL)
        soup = BeautifulSoup(page.content, 'html.parser')
...
# Find links to jobs 
URL_BASE = 'https://au.indeed.com/jobs?q=&l=Australia&sort=date&fromage=1&start='
URL = URL_BASE
...
for start in range(10,20,10):
    try:
        ua = UserAgent()
        header = {'User-Agent':str(ua.chrome),'referer': 'https://au.indeed.com/?from=gnav-viewjob'}
        ...
        for e,x in enumerate(results):
            if "rc/clk?jk" in x.attrs['href'] :
                u = "https://au.indeed.com/viewjob?" + x.attrs['href'].split(r'?')[1].split("&")[0]
                j_urls.add(u)
...
filename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-au.json"
...
################# Addition of the common jobs
    filename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-au.txt"
    jfile = open(filename,'w+', encoding="utf-8")
    jfile.write("[\n")

    for t in range (0,5):
        ydate = datetime.now() - timedelta(t)
        ofilename = "data//"+datetime.strftime(ydate, '%Y-%m-%d')+"-au.json"
    ...
    filename = "ftp//"+datetime.now().strftime("%Y-%m-%d")+"-au.json"
    jfile = open(filename,'w+', encoding="utf-8", newline='\r\n')
    ofilename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-au.txt"
    f_in = open(ofilename, 'r', encoding="utf-8", newline='\r\n')
    x = f_in.readlines()
    for y in x[0:-1]:
