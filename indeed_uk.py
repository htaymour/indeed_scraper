...
def get_job_data(URL):
    try:
        ...
        j_id = URL.replace('https://uk.indeed.com/viewjob?jk=','')
        ...
        if soup.find(id ="originalJobLinkContainer") : 
            ...
            if original_link.find('indeed.com') > -1 :
                try: r = requests.get(original_link,proxies=proxies,headers=header, verify=False,allow_redirects=True,timeout=15)
                except:
                    try: 
                        sleep (2) 
                        r = requests.get(original_link,proxies=proxies,headers=header, verify=False,allow_redirects=True,timeout=15)
                    except : pass
        ...
        if d.find("salaryText") >= 0 : salary = d.split("salaryText:")[1].split(",salaryTextFormatted")[0].replace("\\xa","")
        if d.find("Job Type::") >= 0 : job_type = d.split("Job Type::")[1].split(r"},")[0].replace("[","").replace("]","")
        # **************** SALARY SECTION 
        if salary :  salary_min = salary.split(" ")[0].replace("\\xa3",'')
        if salary.find(" - ") >= 0 : 
            salary_min = salary.split(" - ")[0].replace("\\xa3",'£')
            salary_max = salary.split(" - ")[1].split(' ')[0].replace("\\xa3",'£')
        if salary.find("a ") >= 0 : 
        ...
        if salary.find("\\xa3") >= 0 : 
            salary_currency = "£"
            salary= salary.replace("\\xa3",'£')
        ...
        if desc.upper().find(' UK . ') >= 0 : desc = desc.replace(' UK .','UK.').replace(" uk .","uk.")
...
def get_company_data(company):
    try:
    # global d,soup
        URL = "https://uk.indeed.com/cmp/"+ company.replace(' ','-')  + "/reviews"
        logoUrl = ""
        jobsBoardUrl = "https://uk.indeed.com/cmp/"+ company.replace(' ','-')
        website = ""
        address = ""
        leader_name = ""
        leader_photo = ""
    ...
    try :
        leadership= ""
        URL = "https://uk.indeed.com/cmp/"+ company.replace(' ','-')
        page = requests.get(URL)
        soup = BeautifulSoup(page.content, 'html.parser')
...
# Find links to jobs 
URL_BASE = 'https://uk.indeed.com/jobs?q=&l=United+Kingdom&sort=date&fromage=1&start='
URL = URL_BASE
...
for start in range(10,20,10):
    try:
        ua = UserAgent()
        header = {'User-Agent':str(ua.chrome),'referer': 'https://uk.indeed.com/?from=gnav-viewjob'}
        ...
        for e,x in enumerate(results):
            if "rc/clk?jk" in x.attrs['href'] :
                u = "https://uk.indeed.com/viewjob?" + x.attrs['href'].split(r'?')[1].split("&")[0]
                j_urls.add(u)
...
filename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-uk.json"
...
################# Addition of the common jobs
    filename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-uk.txt"
    jfile = open(filename,'w+', encoding="utf-8")
    jfile.write("[\n")

    for t in range (4,68,8):
        ydate = datetime.now() - timedelta(t/10)
        ofilename = "data\\"+datetime.strftime(ydate, '%Y-%m-%d')+"-uk.json"
        if not os.path.isfile(ofilename) : continue
        f_in = open(ofilename, 'r', encoding="utf-8", newline='\r\n')
        print ("t   ydate  ", str(ydate) , str(t/10))
        x = f_in.readlines()
        jfile.write("{\n")
        for l in x[1:-1]:
    ...
    # filename = "ftp//"+datetime.now().strftime("%Y-%m-%d")+"-uk.json"
    # jfile = open(filename,'w+', encoding="utf-8", newline='\r\n')
    # ofilename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-uk.txt"
    # f_in = open(ofilename, 'r', encoding="utf-8", newline='\r\n')
    # x = f_in.readlines()
    # for y in x[0:-1]:
...
try:
    import gzip
    with open(filename, 'rb') as f_in, gzip.open(filename.replace(".txt",".json.gz"), 'wb+') as f_out:
        f_out.writelines(f_in)
    # os.remove(filename)
    os.system('copy ftp\\'+ filename[5:] + ' ..\\us_scraper\\ftp')
    os.system('copy ' + filename.replace(".txt",".json.gz").replace("//","\\") + ' ..\\us_scraper\\ftp')
