...
def get_job_data(URL):
    try:
        ...
        j_id = URL.replace('https://ca.indeed.com/viewjob?jk=','')
        ...
        if soup.find(id ="originalJobLinkContainer") : 
            ...
            if original_link.startswith('https://www.indeed.com'): 
                r = requests.get(original_link,proxies=proxies,headers=header, verify=False,allow_redirects=True,timeout=150)
                try:
                    if proxies == {} : original_link = r.url
                    elif proxies != {} : original_link = r.headers['sa-final-url']
                except: pass
        ...
        if desc.upper().find(' CA . ') >= 0 : desc = desc.replace(' CA .','CA.').replace(" ca .","ca.")
...
def get_company_data(company):
    try:
    # global d,soup
        URL = "https://ca.indeed.com/cmp/"+ company.replace(' ','-')  + "/reviews"
        logoUrl = ""
        jobsBoardUrl = "https://ca.indeed.com/cmp/"+ company.replace(' ','-')
        website = ""
        address = ""
        leader_name = ""
        leader_photo = ""
    ...
    try :
        leadership= ""
        URL = "https://ca.indeed.com/cmp/"+ company.replace(' ','-')
        page = requests.get(URL)
        soup = BeautifulSoup(page.content, 'html.parser')
...
# Find links to jobs 
URL_BASE = 'https://ca.indeed.com/jobs?q=&l=Canada&fromage=1&start='
URL = URL_BASE

proxies = {}
j_urls = set()
print ("         Collecting Jobs\n ===============================")
for start in range(10,650,10):
    try:
        ua = UserAgent()
        header = {'User-Agent':str(ua.chrome),'referer': 'https://ca.indeed.com/?from=gnav-viewjob'}
        ...
        for e,x in enumerate(results):
            if "rc/clk?jk" in x.attrs['href'] :
                u = "https://ca.indeed.com/viewjob?" + x.attrs['href'].split(r'?')[1].split("&")[0]
                j_urls.add(u)
...
filename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-ca.json"
...
################# Addition of the common jobs
    filename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-ca.txt"
    jfile = open(filename,'w+', encoding="utf-8")
    jfile.write("[\n")

    for t in range (0,5):
        ydate = datetime.now() - timedelta(t)
        ofilename = "data//"+datetime.strftime(ydate, '%Y-%m-%d')+"-ca.json"
    ...
    filename = "ftp//"+datetime.now().strftime("%Y-%m-%d")+"-ca.json"
    jfile = open(filename,'w+', encoding="utf-8", newline='\r\n')
    ofilename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-ca.txt"
    f_in = open(ofilename, 'r', encoding="utf-8", newline='\r\n')
    x = f_in.readlines()
    for y in x[0:-1]:
