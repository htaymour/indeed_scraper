...
def get_job_data(URL):
    try:
        ...
        j_id = URL.replace('https://hk.indeed.com/viewjob?jk=','')
        ...
        if soup.find(id ="originalJobLinkContainer"): 
            ...
            if original_link.find('indeed.com') > -1 :
                try: 
                    r = requests.get(original_link,proxies=proxies,headers=header, verify=False,allow_redirects=True,timeout=350)
                except: 
                    try : sleep (1) ; r = requests.get(original_link,proxies=proxies,headers=header, verify=False,allow_redirects=True,timeout=650)
        ...
        if desc.upper().find(' HK . ') >= 0 : desc = desc.replace(' HK .','HK.').replace(" hk .","hk.")
...
def get_company_data(company):
    try:
    # global d,soup
        URL = "https://hk.indeed.com/cmp/"+ company.replace(' ','-')  + "/reviews"
        logoUrl = ""
        jobsBoardUrl = "https://hk.indeed.com/cmp/"+ company.replace(' ','-')
        website = ""
        address = ""
        leader_name = ""
        leader_photo = ""
    ...
    try :
        leadership= ""
        URL = "https://hk.indeed.com/cmp/"+ company.replace(' ','-')
        page = requests.get(URL)
        soup = BeautifulSoup(page.content, 'html.parser')
...
# Find links to jobs 
URL_BASE = 'https://hk.indeed.com/jobs?q=&l=hong+kong&sort=date&fromage=1&start='
URL = URL_BASE
...
for start in range(10,30,10):
    try:
        ua = UserAgent()
        header = {'User-Agent':str(ua.chrome),'referer': 'https://hk.indeed.com/?from=gnav-viewjob'}
        ...
        for e,x in enumerate(results):
            if "rc/clk?jk" in x.attrs['href'] :
                u = "https://hk.indeed.com/viewjob?" + x.attrs['href'].split(r'?')[1].split("&")[0]
                j_urls.add(u)
...
filename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-hk.json"
...
################# Addition of the common jobs
    filename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-hk.txt"
    jfile = open(filename,'w+', encoding="utf-8")
    jfile.write("[\n")

    for t in range (0,5):
        ydate = datetime.now() - timedelta(t)
        ofilename = "data//"+datetime.strftime(ydate, '%Y-%m-%d')+"-hk.json"
    ...
    filename = "ftp//"+datetime.now().strftime("%Y-%m-%d")+"-hk.json"
    jfile = open(filename,'w+', encoding="utf-8", newline='\r\n')
    ofilename = "data//"+datetime.now().strftime("%Y-%m-%d")+"-hk.txt"
    f_in = open(ofilename, 'r', encoding="utf-8", newline='\r\n')
    x = f_in.readlines()
    for y in x[0:-1]:
